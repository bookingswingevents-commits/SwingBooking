...
export default function ArtistMediaKitPage() {
    ...
    (async () => {
      try {
        ...
        const { data: reqData, error: reqErr } = await supabase
          ...
          .select('id, title, formation, event_format, venue_company_name, venue_address, practical_info, venue_contact_email, venue_contact_phone')
          .eq('id', id)
          .maybeSingle();
        if (reqErr || !reqData) throw new Error(reqErr?.message || 'Demande introuvable');
        setReq(reqData as BookingRequest);
        setVenueInfo({
          venue_company_name: (reqData as any)?.venue_company_name ?? null,
          venue_address: (reqData as any)?.venue_address ?? null,
          venue_contact_email: (reqData as any)?.venue_contact_email ?? null,
          venue_contact_phone: (reqData as any)?.venue_contact_phone ?? null,
        });
        ...
        const { data: artistRow, error: artistErr } = await supabase
          .from('artists')
          .select('stage_name, bio, instagram_url, facebook_url, youtube_url')
          .eq('id', session.user.id)
          .maybeSingle();
        if (!artistErr && artistRow) setArtist(artistRow as ArtistProfile);
      } catch (e: any) {
  ...
  const instaPost = `${baseText}\n\n${artist?.stage_name || 'Artiste'} en live !\n${hashtags}`;
  ...
  const sendCommunicationRequest = async () => {
    try {
      ...
      const res = await fetch('/api/communication-requests/create', {
        ...
        body: JSON.stringify({ requestId: id, role: 'artist', notes }),
      });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <header className="space-y-1">
        <h1 className="text-2xl font-bold">Kit de communication (artiste)</h1>
        <p className="text-slate-600 text-sm">
          Prépare tes posts pour cet événement.
        </p>
      </header>
      ...
      <section className="border rounded-2xl p-4 space-y-2">
        ...
        <div className="text-sm space-y-1">
          ...
          {venueInfo?.venue_company_name && <div>{venueInfo.venue_company_name}</div>}
        ...
        {occurrences.length > 0 && (
          <ul className="text-sm text-slate-700 space-y-1">
            {occurrences.map((o) => (
              <li key={o.id}>
                {fmtDateFR(o.date)} {o.start_time || ''} — {o.address_snapshot || venueInfo?.venue_address}
              </li>
            ))}
          </ul>
        )}
      ...
      <section className="border rounded-2xl p-4 space-y-2">
        ...
        {artist ? (
          ...
          <div className="text-sm text-slate-600">Profil artiste indisponible.</div>
        )}
      </section>
      ...
      <section className="border rounded-2xl p-4 space-y-3 bg-amber-50 border-amber-200">
        ...
        <p className="text-xs text-amber-800">
          Nous vous contacterons via votre email de compte pour finaliser.
        </p>
      </section>
    </div>
  );
