...
export default function VenueRoadmapDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (!session) {
          setError('Connectez-vous pour voir la feuille de route.');
          setLoading(false);
          return;
        }
  ...
  if (error || !req) {
    return (
      <div className="space-y-3">
        ...
        <Link href="/venue/roadmaps" className="text-[var(--brand)] underline text-sm">
          ← Retour
        </Link>
      </div>
    );
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <header className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/venue/roadmaps" className="text-sm underline text-[var(--brand)]">
            ← Retour
          </Link>
        </div>
      </header>
