...
export default function VenueRoadmapsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // 1) Récupérer les demandes liées au client (par venue_id ou contact_email)
        const { data: reqs, error: reqErr } = await supabase
          .from('booking_requests')
          .select('id, title, event_date, start_time, duration_minutes, event_format, formation, venue_address, venue_company_name, venue_contact_email, venue_id')
          .or(`venue_id.eq.${userId},venue_contact_email.eq.${session.user.email ?? ''}`)
        ...
        // 2) Itinéraires liés à ces demandes
        ...
        <ul className="space-y-3">
          {rows.map((r) => (
            <li key={r.id} className="border rounded-2xl p-4 hover:shadow-sm space-y-2">
              ...
              <Link
                href={`/venue/requests/${r.request_id}`}
