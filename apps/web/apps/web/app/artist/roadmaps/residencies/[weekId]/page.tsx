...
export default function ArtistResidencyRoadmapPage() {
    ...
    (async () => {
      try {
        ...
        const { data: booking } = await supabase
          ...
          .maybeSingle();
        const wk = booking?.residency_weeks as WeekRow | null;
        if (!wk) {
        ...
        const residencyId = wk.residencies?.id;
  ...
  const clientName = useMemo(() => {
    const res = week?.residencies;
    return Array.isArray(res?.clients) ? res?.clients[0]?.name : res?.clients?.name ?? null;
  }, [week]);
        ...
        <div>
          ...
          {week ? (
            <p className="text-sm text-slate-600">
              {week.residencies?.name ?? 'Programmation'}
